...
void PerlinNoise3D_float(float3 input, out float Out)
{
	...
	float amplitude = 1.0;
	float frequency = 1.0;
	const int OCTAVES = 5; // you can change number of octaves

	for (int i = 0; i < OCTAVES; i++)
	{
		float3 p = input * frequency;
		float noiseVal = cnoise(p); // scaled period
...
void PerlinNoise3DPeriodic_float(float3 input, float3 period, out float Out)
{
	...
	{
		float3 p = input * frequency;
		float noiseVal = pnoise(p, period * frequency); // scaled period
		total += noiseVal * amplitude;
