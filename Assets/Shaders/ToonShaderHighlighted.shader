Shader "Custom/ToonShaderHighlighted"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 HightlightColor(half4 c)
            {
                ...
                hsv.x = frac(hsv.x + _Time.x * 5.0);
                
                float4 K2 = float4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
                float3 p2 = abs(frac(hsv.xxx + K2.xyz) * 6.0 - K2.www);
                return half4(hsv.z * lerp(K2.xxx, saturate(p2 - K2.xxx), hsv.y), c.a);
            }

            half4 GetShadowColor(half shadow, float2 uv)
            {
                half4 tex = SAMPLE_TEXTURE2D(_ShadowMap, sampler_ShadowMap, uv);
                half4 shadowColor = (0.4 + 0.6 * tex) * HightlightColor(_ShadowColor);

                return shadow * HightlightColor(_MainColor) + (1-shadow) * shadowColor;
            }
            half4 GetSurfaceColor(float NdotL)
            {
                if(NdotL > 0.95)return HightlightColor(_HighlightColor);
                return HightlightColor(_MainColor);
            }
