...
The way these effects are achieved is:
 ...
 - Mario Paint thing wiggle: Do radial circles using sine wave and distance from "point," which I set to center. Convert these wiggles to offset UVs -> plug that into screen color.
 - Pixelate: Multiply UV by the number of pixel buckets. Floor it. Divide back. This had to be done in code, so we can treat color as a vector without a ceiling like color.
 - Change Material: Get a reference to the full screen renderer and simply set its pass material value.
Showcase Gif:
![](Final-Gif.gif)
