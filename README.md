...
Developed a comprehensive toon surface shader with the following features:

- **Multiple Light Support**
  - Implemented support for main directional light and additional light sources
  - Proper shadow attenuation and distance calculations
  - Ambient lighting integration using spherical harmonics
- **Interesting Shadow: Ink Wash Shadow Texture**
  - Created custom seamless shadow texture mimicking traditional ink brush strokes
  - Implemented **tri-planar mapping** to project shadow texture onto geometry from three axes
  - Combined with procedural noise for organic, hand-painted shadow appearance
  - UV-based shadow mapping that conforms to object geometry
  - Exposed "Shadow Scale" parameter for per-material texture tiling control

#### Special Surface Shader: Vertex Animation Effect

Created an enhanced shader variant with dynamic motion characteristics:

- **Technical Approach:** Vertex position animation
  - Implemented time-based vertex position offset
  - Used sine function to drive vertex movement

### Outlines

#### Render Feature System Setup

- **Full Screen Feature Debugging**
  - Fixed bug in base code's Full Screen Feature to properly apply fullscreen shaders
  - Successfully configured render pipeline for fullscreen post-processing effects
- **Normal Buffer Implementation**
  - Configured Normal Feature render pass to render scene normals to dedicated render texture
  - Created and configured Normal Buffer render texture with resolution matching game window
  - Properly connected Normal Copy material and render target
- **Depth Buffer Access**
  - Enabled URP depth buffer option
  - Successfully accessed depth and normal information in Shader Graph

#### Post-Processing Outline Implementation

- **Sobel Edge Detection**
  - Implemented edge detection algorithm based on Sobel operator
  - Processes both depth buffer and normal buffer to detect geometric edges and interior details
  - Adjustable outline width parameter
- **Animated Outline Effect**
  - Implemented dynamic outline thickness variation based on time node
  - Outline width changes periodically over time, creating hand-drawn dynamic feel
  - Enhances visual appeal and artistic expressiveness
- **Per-Object Rendering System**
  - Extended Render Feature functionality to support independent outline rendering for different objects
  - Implemented multi-layer outline rendering system to enhance scene visual hierarchy

### Full Screen Post-Process Effect

Implemented paper texture post-processing effect:

- **Paper Texture Overlay**
  - Achieved paper texture overlay through fullscreen Shader Graph

### Interactivity

Implemented real-time material switching system to showcase dynamic visual changes:

- **Dual Renderer Type Support**
  - Developed material switching script compatible with both `MeshRenderer` and `SkinnedMeshRenderer`
  - Automatically detects object's renderer type and applies corresponding switching logic
- **Keyboard Interaction**
  - **F2 Key:** Switches material style for `SkinnedMeshRenderer` objects
  - **Space Key:** Switches material style for `MeshRenderer` objects
- **Technical Implementation**
  - Material array management system
  - Index-based cyclic switching logic
  - Runtime renderer component detection and material updates

---

## Extra Credit

### Procedural Texture Coloring

Implemented advanced procedural shading system beyond simple value multiplication:

- **Ramp-based shading with custom blending logic**
  - Multi-step ramp sampling with procedural noise offset
  - Applied **Gaussian blur smoothing** to ramp transitions for soft watercolor effects
  - Adjustable blur radius and sampling steps to control softness
  - Interior noise mapping adds subtle brush texture variation

### Advanced Render Feature System

- Extended Render Feature functionality to support independent outline rendering for different objects
- Implemented multi-layer outline rendering system, enhancing scene visual hierarchy and control flexibility
